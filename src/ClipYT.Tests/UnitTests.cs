...
namespace ClipYT.Tests
{
    public class UnitTests
    {
        ...
        [InlineData("https://www.instagram.com/p/DAEQq8lvpvD/")]
        [InlineData("https://www.facebook.com/reel/713709415093896")]
        public async Task ThumbnailService_Should_Return_Valid_Thumbnail_Url(string inputUrl)
        {
            // Arrange
            var httpClientFactory = new HttpClientFactory();
            var thumbnailService = new ThumbnailService(httpClientFactory);
            var url = new Uri(inputUrl);

            // Act
            var thumbnailUrl = await thumbnailService.GetThumbnailUrlAsync(url);
        ...
        [InlineData("https://www.instagram.com/p/DAEQq8lvpvD/")]
        [InlineData("https://www.facebook.com/reel/713709415093896")]
        public async Task ThumbnailService_Should_Return_Thumbnail_For_Social_Media_Urls(string inputUrl)
        {
            // Arrange
            var httpClientFactory = new HttpClientFactory();
            var thumbnailService = new ThumbnailService(httpClientFactory);
            ...
            Assert.True(Uri.TryCreate(thumbnailUrl, UriKind.Absolute, out _));
        }
